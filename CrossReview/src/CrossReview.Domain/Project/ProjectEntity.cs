...
public class ProjectEntity
{
    ...
    private List<ReviewPeriod> _reviewPeriods;
    ...
    public IReadOnlyCollection<ReviewPeriod> ReviewPeriods => _reviewPeriods;
    ...
    {
        var period = _reviewPeriods.Find(p => p.Id == periodId);
        ...
        if (period.Status == EnumReviewPeriodStatus.Active &&
            _reviewPeriods.Any(p => p.Status == EnumReviewPeriodStatus.Active))
            throw new ValidationException("Нельзя добавить еще один активный период");

        period.Activate();
    }

    public void EnsureUsersCanReviewEachOther(Guid reviewerId, Guid revieweeId)
    {
        var reviewee = Members.FirstOrDefault(m => m.UserId == revieweeId);
        var reviewer = Members.FirstOrDefault(m => m.UserId == reviewerId);
    ...
    public void EnsureTemplateBelongsToProject(Guid templateProjectId)
    {
        if (templateProjectId != Id)
            throw new ValidationException("Шаблон не принадлежит проекту");
    }
}